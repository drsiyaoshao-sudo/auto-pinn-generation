...
Examples:
  /hear "PINN physics-first vs data-first training order" "physics-dominant warmup required" vs "data fitting first, physics as regulariser"
  /hear "L_ODE removal" "remove L_ODE — inconsistent with walker_model" vs "keep L_ODE — physics grounding required by Article I"
  /hear "Chebyshev vs Fourier embedding basis" "Chebyshev T1-T5 on t — cadence-grounded" vs "Fourier B matrix on (x,t) — spectral coverage"
...
- Verify agent roster (attorney-A, attorney-B, simulator-operator, plotter, uart-reader, pinn-executor)
...
Constitutional grounding to check before arguing:
  docs/gaitsense_code/amendments.md       — all ratified amendments (1–21)
  docs/gaitsense_code/case_law.md         — all recorded precedents
  docs/gaitsense_code/bills/              — all enacted bills (including this session's)
...
Evidence commands available during this hearing:
  ...
  /plot-evidence training <run_id>        — PINN loss curve
  /plot-evidence sim <profile>            — full simulation evidence
  /plot-evidence uart <log_path>          — UART session output

Lesson precedents relevant to PINN hearings:
  docs/gaitsense_code/lesson_nine_run_diagnostic.md   — Amendment 21 checklist
  docs/gaitsense_code/lesson_code_pasta_prevention.md — agent scope boundaries
══════════════════════════════════════════════════════════════
```
...
For PINN-related hearings, invoke pinn-executor for trial training evidence if the
Justice asks for it. pinn-executor may also be invoked as a Bureaucracy agent to
run Amendment 21 pre-training diagnostics as evidence.
